...
class End2End:
    ...
    def training_iteration(self, data, device, model, criterion_seg, criterion_dec, optimizer, weight_loss_seg, weight_loss_dec,
        ...
        images, seg_mask1s, seg_mask2s, seg_mask3s, seg_loss_mask1s, seg_loss_mask2s, seg_loss_mask3s, is_segmented, _ = data
        #print (data[0])
        ...
        for sub_iter in range(num_subiters):
            ...
            seg_masks_1 = seg_mask1s[sub_iter * memory_fit:(sub_iter + 1) * memory_fit, :, :, :].to(device)
            seg_masks_2 = seg_mask2s[sub_iter * memory_fit:(sub_iter + 1) * memory_fit, :, :, :].to(device)
            seg_masks_3 = seg_mask3s[sub_iter * memory_fit:(sub_iter + 1) * memory_fit, :, :, :].to(device)
            #seg_loss_masks_ = seg_loss_masks[sub_iter * memory_fit:(sub_iter + 1) * memory_fit, :, :, :].to(device)
            seg_loss_masks_1 = seg_loss_mask1s[sub_iter * memory_fit:(sub_iter + 1) * memory_fit, :, :, :].to(device)
            seg_loss_masks_2 = seg_loss_mask2s[sub_iter * memory_fit:(sub_iter + 1) * memory_fit, :, :, :].to(device)
            seg_loss_masks_3 = seg_loss_mask3s[sub_iter * memory_fit:(sub_iter + 1) * memory_fit, :, :, :].to(device)

            #is_pos_ = seg_masks_.max().reshape((memory_fit, 1)).to(device)
            is_pos_1 = seg_masks_1.max().reshape((memory_fit, 1)).to(device)
            # is_pos_2 = seg_masks_2.max().reshape((memory_fit, 1)).to(device)
            # is_pos_3 = seg_masks_3.max().reshape((memory_fit, 1)).to(device)
            ...
            if tensorboard_writer is not None and iter_index % 100 == 0:
                ...
                tensorboard_writer.add_image(f"{iter_index}/seg_mask1", seg_mask1s[0, :, :, :])
                tensorboard_writer.add_image(f"{iter_index}/seg_mask2", seg_mask2s[0, :, :, :])
                tensorboard_writer.add_image(f"{iter_index}/seg_mask3", seg_mask3s[0, :, :, :])
                #tensorboard_writer.add_image(f"{iter_index}/seg_loss_mask", seg_loss_masks_[0, :, :, :])
                tensorboard_writer.add_image(f"{iter_index}/seg_loss_mask1", seg_loss_masks_1[0, :, :, :])
                tensorboard_writer.add_image(f"{iter_index}/seg_loss_mask2", seg_loss_masks_2[0, :, :, :])
                tensorboard_writer.add_image(f"{iter_index}/seg_loss_mask3", seg_loss_masks_3[0, :, :, :])

            #decision, output_seg_mask= model(images_)
            decision, output_seg_mask1, output_seg_mask2, output_seg_mask3 = model(images_)

            #print(is_segmented[sub_iter], part[sub_iter])
            ...
            if is_segmented[sub_iter]:
                if self.cfg.WEIGHTED_SEG_LOSS:
                    ...
                    loss_seg1 = torch.mean(criterion_seg(output_seg_mask1, seg_masks_1) * seg_loss_masks_1)
                    loss_seg2 = torch.mean(criterion_seg(output_seg_mask2, seg_masks_2) * seg_loss_masks_2)
                    loss_seg3 = torch.mean(criterion_seg(output_seg_mask3, seg_masks_3) * seg_loss_masks_3)
                    #print(1)
                else:
                    #loss_seg = criterion_seg(output_seg_mask, seg_masks_)
                    loss_seg1 = criterion_seg(output_seg_mask1, seg_masks_1)
                    loss_seg2 = criterion_seg(output_seg_mask2, seg_masks_2)
                    loss_seg3 = criterion_seg(output_seg_mask3, seg_masks_3)
                    #print(2)
                #loss_dec = criterion_dec(decision, is_pos_)
                loss_dec1 = criterion_dec(decision, is_pos_1)
                # loss_dec2 = criterion_dec(decision, is_pos_2)
                # loss_dec3 = criterion_dec(decision, is_pos_3)

                #total_loss_seg += loss_seg.item()
                total_loss_seg += loss_seg1.item() +loss_seg2.item() +loss_seg3.item()
                #total_loss_dec += loss_dec.item()
                total_loss_dec += loss_dec1.item()#+loss_dec2.item()+loss_dec3.item()

                total_correct += (decision > 0.0).item() == is_pos_1.item()
                #loss = weight_loss_seg * loss_seg + weight_loss_dec * loss_dec
                loss = weight_loss_seg * (loss_seg1 +loss_seg2 +loss_seg3) + weight_loss_dec * loss_dec1

            else:
                loss_dec = criterion_dec(decision, is_pos_1)
                total_loss_dec += loss_dec.item()
    ...
    def eval_model(self, device, model, eval_loader, save_folder, save_images, is_validation, plot_seg):
        ...
        for data_point in eval_loader:
            ...
            image, seg_mask1, seg_mask2, seg_mask3, seg_loss_mask1, seg_loss_mask2, seg_loss_mask3, _, sample_name = data_point
            image, seg_mask1,seg_mask2,seg_mask3 = image.to(device), seg_mask1.to(device), seg_mask2.to(device), seg_mask3.to(device)
            is_pos1 = (seg_mask1.max() > 0).reshape((1, 1)).to(device).item()
            # is_pos2 = (seg_mask2.max() > 0).reshape((1, 1)).to(device).item()
            # is_pos3 = (seg_mask3.max() > 0).reshape((1, 1)).to(device).item()
            #prediction, pred_seg = model(image)
            prediction, pred_seg1, pred_seg2, pred_seg3 = model(image)
            pred_seg1 = nn.Sigmoid()(pred_seg1)
            pred_seg2 = nn.Sigmoid()(pred_seg2)
            pred_seg3 = nn.Sigmoid()(pred_seg3)
            ...
            pred_seg3 = pred_seg3.detach().cpu().numpy()
            #seg_mask = seg_mask.detach().cpu().numpy()
            seg_mask1 = seg_mask1.detach().cpu().numpy()
            seg_mask2 = seg_mask2.detach().cpu().numpy()
            seg_mask3 = seg_mask3.detach().cpu().numpy()


            predictions.append(prediction)
            # predictions.extend([prediction] * 3)
            #ground_truths.append(is_pos)
            ground_truths.append(is_pos1)
            # ground_truths.append(is_pos2)
            # ground_truths.append(is_pos3)
            #res.append((prediction, None, None, is_pos, sample_name[0]))
            res.append((prediction, None, None, is_pos1, sample_name[0]))
            # res.append((prediction, None, None, is_pos2, sample_name[0]))
            # res.append((prediction, None, None, is_pos3, sample_name[0]))

            if not is_validation:
                if save_images:
                    image = cv2.resize(np.transpose(image[0, :, :, :], (1, 2, 0)), dsize)
                    image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
                    pred_seg1 = cv2.resize(pred_seg1[0, 0, :, :], dsize) if len(pred_seg1.shape) == 4 else cv2.resize(pred_seg1[0, :, :], dsize)
                    pred_seg2 = cv2.resize(pred_seg2[0, 0, :, :], dsize) if len(pred_seg2.shape) == 4 else cv2.resize(pred_seg2[0, :, :], dsize)
                    pred_seg3 = cv2.resize(pred_seg3[0, 0, :, :], dsize) if len(pred_seg3.shape) == 4 else cv2.resize(pred_seg3[0, :, :], dsize)

                    #seg_mask = cv2.resize(seg_mask[0, 0, :, :], dsize)
                    seg_mask1 = cv2.resize(seg_mask1[0, 0, :, :], dsize)
                    seg_mask2 = cv2.resize(seg_mask2[0, 0, :, :], dsize)
                    seg_mask3 = cv2.resize(seg_mask3[0, 0, :, :], dsize)
                    if self.cfg.WEIGHTED_SEG_LOSS:
                        #seg_loss_mask = cv2.resize(seg_loss_mask.numpy()[0, 0, :, :], dsize)
                        seg_loss_mask1 = cv2.resize(seg_loss_mask1.numpy()[0, 0, :, :], dsize)
                        seg_loss_mask2 = cv2.resize(seg_loss_mask2.numpy()[0, 0, :, :], dsize)
                        seg_loss_mask3 = cv2.resize(seg_loss_mask3.numpy()[0, 0, :, :], dsize)

                        #utils.plot_sample(sample_name[0], image, pred_seg, seg_loss_mask, save_folder, decision=prediction, plot_seg=plot_seg)
                        utils.plot_sample(sample_name[0], image, pred_seg1, seg_loss_mask1, pred_seg2, seg_loss_mask2, pred_seg3, seg_loss_mask3, save_folder, decision=prediction, plot_seg=plot_seg)

                    else:
                        #utils.plot_sample(sample_name[0], image, pred_seg, seg_mask, save_folder, decision=prediction, plot_seg=plot_seg)
                        utils.plot_sample(sample_name[0], image, pred_seg1, seg_mask1, pred_seg2, seg_mask2, pred_seg3, seg_mask3, save_folder, decision=prediction, plot_seg=plot_seg)
    ...
    def _save_train_results(self, results):
        ...
        plt.plot(le, ld, label="Loss dec", color='yellow')
        plt.ylim(bottom=0)
        plt.grid()
        plt.xlabel("Epochs")
        plt.legend()
    ...
    # def _get_optimizer(self, model):
    #     return torch.optim.AdamW(model.parameters(), self.cfg.LEARNING_RATE)
    # def _get_optimizer(self, model):
